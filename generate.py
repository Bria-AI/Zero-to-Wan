...
from models.wan.configs import MAX_AREA_CONFIGS, SIZE_CONFIGS, SUPPORTED_SIZES, WAN_CONFIGS
from models.wan.image2video import WanI2V
from models.wan.text2video import WanT2V
from models.wan.utils.prompt_extend import DashScopePromptExpander, QwenPromptExpander
from models.wan.utils.utils import cache_image, cache_video, str2bool
...
def _validate_args(args):
    # Basic check
    assert args.original_ckpt_dir is not None, "Please specify the original checkpoint directory."
    assert args.finetuned_ckpt_dir is not None, "Please specify the finetuned checkpoint directory."
...
def _parse_args():
    ...
    parser.add_argument(
        "--finetuned_ckpt_dir", type=str, default=None, help="The path to the finetuned checkpoint directory."
    )
    parser.add_argument(
        "--original_ckpt_dir", type=str, default=None, help="The path to the original checkpoint directory."
    )
    parser.add_argument(
        "--offload_model",
        type=str2bool,
...
def generate(args):
    ...
    if "t2v" in args.task or "t2i" in args.task:
        ...
        wan_t2v = WanT2V(
            config=cfg,
            original_checkpoint_dir=args.original_ckpt_dir,
            finetuned_checkpoint_dir=args.finetuned_ckpt_dir,
            device_id=device,
            rank=rank,
